...
(library (wak texinfo string-utils)
...
;; performs a replacement on each character for which matches are
;; found.
...
;; @end lisp"
(define* (transform-string str match? replace (start 0) (end (string-length str)))
    ...
    (lambda (os)
      (let ((matcher
             (cond ((char? match?)
                   ...
                   ((boolean? match?)
                    (lambda (c) match?))
                   (else (assertion-violation 'transform-string
                                              "expected #t, char, string, or procedure"
                                              match?))))
...
;; @end lisp
(define* (expand-tabs str (tab-size 8))
  (transform-string str #\tab (make-string tab-size #\space)))

;;@ Returns a copy of @var{str} with all given special characters
;; preceded by the given @var{escape-char}.
...
;; characters.  Any needed padding is done by character @var{chr},
;; which defaults to @samp{#\\space}.  If @var{rchr} is provided, then
;; the padding to the right will use it instead.  See the examples
;; below.  left and @var{rchr} on the right.  The default @var{width}
;; is 80.  The default @var{lchr} and @var{rchr} is @samp{#\\space}.
...
;; => \"<<<<< Richard Todd >>>>>\"
;; @end lisp
(define* (center-string str (width 80) (chr #\space) (rchr #f))
...
;; is never truncated."
(define* (left-justify-string str (width 80) (chr #\space))
  (let* ((len (string-length str))
...
(define* (right-justify-string str (width 80) (chr #\space))
  (let* ((len (string-length str))
         (pad (make-string (max (- width len) 0) chr)))
    (if (>= len width)
        str
...
;; => \"H-e--l--l--o\"
;; @end lisp"
(define* (collapse-repeated-chars str (chr #\space) (num 1))
...
(define* (make-text-wrapper options)
  (let-options* options ((line-width 80)
                         (expand-tabs? #t)
                         (tab-width 8)
                         ...
                         (break-long-words? #t))
    (lambda (str)
      (let* ((transformed (transform-string str #\newline #\space))
             (expanded (if expand-tabs?
                           (expand-tabs transformed tab-width)
                           transformed))
             (collapsed (if collapse-whitespace?
                            (collapse-repeated-chars expanded)
                            expanded))
             (str (string-trim str)))
          ...
          (if (null? words)
              ...
              (let ((length-left (- line-width
                ...
                (cond 
                  ...
                  ;; well, then is it the first word and we
                  ;; *shouldn't* break long words, then...
                  (else
...
;;@ Wraps the text given in string @var{str} according to the
;; parameters provided in @var{keywds}, or the default setting if they
;; are not given. Returns a list of strings representing the formatted
;; lines. Valid keyword arguments are discussed in
;; @code{make-text-wrapper}."
(define (string->wrapped-lines str options)
  ((make-text-wrapper options) str))

;;@ Wraps the text given in string @var{str} according to the
;; parameters provided in @var{keywds}, or the default setting if they
;; are not given.  Returns a single string with the wrapped text.
;; Valid keyword arguments are discussed in @code{make-text-wrapper}."
(define (fill-string str options)
