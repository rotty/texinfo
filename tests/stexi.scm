...
(import (rnrs)
        (spells condition)
        (only (spells strings) string-null?)
        (texinfo private)
        (test-manager))

(define nl (string #\newline))

(in-test-group read-verbatim-body
...
(in-test-group read-arguments
  ...
  (define-each-test
    ...
    (test "foo,,bar}" '("foo" #f "bar"))
    (assert-exception condition? (test "foo,,bar" 'foo))))

(in-test-group complete-start-command
  ...
  (define-each-test
    ...
    (assert-exception condition? (test 'emph "no brace here"))
    (assert-equal
     '(emph () INLINE-TEXT)
...
(in-test-group read-char-data
  (let* ((code (make-texinfo-token 'START 'code))
    ...
    (define-each-test
      ...
      (assert-exception condition? (test "" #f #f '() (eof-object)))
      (test "  " #t #f '("  ") (eof-object))
...
(in-test-group texinfo->stexinfo
  (define (test str expected-res)
  ...
  (define-each-test
    (assert-exception condition? (test "@dots{}\n" '(texinfo (dots))))
    ...
    (assert-exception condition? (try-with-title "my title" "@dots{arg}"))
